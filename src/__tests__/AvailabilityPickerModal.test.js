...
describe('AvailabilityPickerModal - 重新打開時要用最新的 initialRanges 重新預填', () => {
  ...
  test('modelValue 從 false 變 true 時，重新套用最新的 initialRanges', async () => {
    ...
    await wrapper.setProps({
      initialRanges: [{ date: '2026-08-01', from: '09:00', to: '10:00' }],
    })
    ...
    expect(wrapper.vm.selectedDates).toEqual({
      '2026-08-01': [{ from: '09:00', to: '10:00', endTimeUserSet: true }],
    })
  ...
  test('情境四修改報名時段：重開 picker 後，未重新勾選但先前已選的候選日期仍保留，不會被覆蓋成沒選', async () => {
    ...
    await wrapper.setProps({
      initialRanges: [
        { date: '2026-08-01', from: '09:00', to: '10:00' },
        { date: '2026-08-15', from: '14:00', to: '15:00' },
      ],
    })
    await wrapper.setProps({ modelValue: true })
...
describe('AvailabilityPickerModal - dateOnly 與 allowedDates', () => {
  ...
  test('Date-only picker shows fixed activity time and date-language title', () => {
    const wrapper = mountDateOnly({ fixedTimeLabel: '19:00 – 21:00' })

    expect(wrapper.text()).not.toContain('選取有空時間')
    expect(wrapper.text()).toContain('選擇可參加日期')
    expect(wrapper.text()).toContain('活動時間：19:00 – 21:00')
...
describe('AvailabilityPickerModal - timeWindow 限制可選小時', () => {
  test('傳入 timeWindowStart/timeWindowEnd 時，時間選取下拉選單只列出範圍內的小時', async () => {
    ...
    expect(options).toEqual(['10:00', '11:00', '12:00', '13:00'])
  })
...
describe('AvailabilityPickerModal - 頂部標題依情境顯示正確內容', () => {
  test('fixedDate 且有設時段範圍時，顯示活動時間而非活動日期範圍', () => {
    ...
    expect(wrapper.text()).toContain('13:00 – 17:00')
    expect(wrapper.text()).not.toContain('活動日期範圍')
  })
...
describe('AvailabilityPickerModal - 結束時間選單排除等於或早於已選開始時間的選項', () => {
  test('選了開始時間後，結束時間選單不會出現該小時或更早的選項', async () => {
    ...
    const startButtons = wrapper.findAll('.time-picker-wrap button')
    await startButtons[0].trigger('click')
    const startOption = wrapper.findAll('button[data-hour]').find((o) => o.text() === '01:00')
    ...
    expect(endOptions).not.toContain('00:00')
    expect(endOptions).not.toContain('01:00')
    expect(endOptions[0]).toBe('02:00')
  })
  ...
  test('同一天有兩筆 range 時，各自的結束時間選單只依自己的開始時間過濾', async () => {
    ...
    await startButtons[0].trigger('click')
    let startOption = wrapper.findAll('button[data-hour]').find((o) => o.text() === '08:00')
    await startOption.trigger('click')
    ...
    startOption = wrapper.findAll('button[data-hour]').find((o) => o.text() === '15:00')
    await startOption.trigger('click')
    ...
    expect(secondRangeEndOptions).not.toContain('08:00')
    expect(secondRangeEndOptions[0]).toBe('16:00')
  })
})
...
describe('AvailabilityPickerModal - 報名時段不可以重疊或重複', () => {
  ...
  test('第一筆時段結束時間改短後，點「+ 新增時段」新增的第二筆不會跟第一筆重疊', async () => {
    ...
    let startButtons = wrapper.findAll('.time-picker-wrap button')
    await startButtons[0].trigger('click')
    let startOption = wrapper.findAll('button[data-hour]').find((o) => o.text() === '18:00')
    await startOption.trigger('click')

    const endButtons = wrapper.findAll('.time-picker-wrap button')
    await endButtons[1].trigger('click')
    const endOption = wrapper.findAll('button[data-hour]').find((o) => o.text() === '21:00')
...
describe('AvailabilityPickerModal - 開始時間選單排除今天已經過去的時段', () => {
  ...
  test('正在編輯的日期是今天時，開始時間選單只列出目前小時之後的選項', async () => {
    ...
    expect(startOptions).not.toContain('09:00')
  ...
  test('正在編輯的日期不是今天時，開始時間選單不受目前時間影響', async () => {
    ...
    expect(startOptions[0]).toBe('00:00')
  })
})
...
describe('AvailabilityPickerModal - 選好開始時間後結束時間自動帶入 +1 小時', () => {
  test('選開始時間會自動把結束時間帶成 +1 小時', async () => {
    ...
    let startButtons = wrapper.findAll('.time-picker-wrap button')
    await startButtons[0].trigger('click')
    const startOption = wrapper.findAll('button[data-hour]').find((o) => o.text() === '09:00')
    await startOption.trigger('click')

    startButtons = wrapper.findAll('.time-picker-wrap button')
    expect(startButtons[1].text()).toBe('10:00')
  })
  ...
  test('手動選過結束時間後，改開始時間不會覆蓋（仍合理的情況下）', async () => {
    ...
    let option = wrapper.findAll('button[data-hour]').find((o) => o.text() === '09:00')
    await option.trigger('click')

    buttons = wrapper.findAll('.time-picker-wrap button')
    await buttons[1].trigger('click')
    option = wrapper.findAll('button[data-hour]').find((o) => o.text() === '15:00')
    await option.trigger('click')

    buttons = wrapper.findAll('.time-picker-wrap button')
    await buttons[0].trigger('click')
    option = wrapper.findAll('button[data-hour]').find((o) => o.text() === '10:00')
    await option.trigger('click')

    buttons = wrapper.findAll('.time-picker-wrap button')
    expect(buttons[1].text()).toBe('15:00')
  })
})
...
describe('AvailabilityPickerModal - dateWindows 依日期限制可選小時（情境四）', () => {
  test('單一窗口時下拉選單只提供窗口內的小時', async () => {
    ...
    const options = wrapper.findAll('button[data-hour]').map((o) => o.text())
    expect(options).toEqual(['14:00', '15:00'])
  })
  ...
  test('候選時段窗口 01:00-07:00 時，開始時間選單不能選到窗口終點 07:00，選 06:00 自動帶入的結束時間 clamp 在 07:00', async () => {
    ...
    expect(startOptionTexts).not.toContain('07:00')

    const startOption = wrapper.findAll('button[data-hour]').find((o) => o.text() === '06:00')
    await startOption.trigger('click')

    startButtons = wrapper.findAll('.time-picker-wrap button')
    // clamp 在窗口終點 07:00，不是天真的 6:00+1小時算出來的值（這裡剛好也是 7:00，
    // 但關鍵是上面已經確認 07:00 選不到，所以這個結果是 clamp 出來的，不是選到終點再 +1）
    expect(startButtons[1].text()).toBe('07:00')
...
describe('AvailabilityPickerModal - dateWindows 預設值與整天分支（情境四）', () => {
  ...
  test('選取有 dateWindows 的候選日期，預設帶入該窗口的完整範圍', async () => {
    // 候選日期的窗口（14:00–16:00）還沒過去，才會是一個能點選的日期格子
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 5, 12, 9, 30))
    ...
    expect(wrapper.emitted('confirm')[0][0]).toEqual([
      {
        date: '2026-06-12',
        allDay: false,
        timeRanges: [{ from: '14:30', to: '15:30', endTimeUserSet: true }],
      },
    ])
  })
})
...
describe('AvailabilityPickerModal - 已選清單一次標出所有有問題的日期', () => {
  ...
  test('有問題的日期，已選清單的 chip 會有錯誤標記', async () => {
    ...
    expect(badChip.classes().join(' ')).toContain('bujo-danger')
    expect(goodChip.classes().join(' ')).not.toContain('bujo-danger')
  })
})
...
describe('AvailabilityPickerModal - 候選日期格子過期停用（isExpired）', () => {
  afterEach(() => {
    vi.useRealTimers()
  })

  function findDayCell(wrapper, day) {
    return wrapper
      .find('.grid-cols-7')
      .findAll('div')
      .find((cell) => cell.text() === String(day))
  ...
  test('情境四逐日窗口（dateWindows[date].end）已過去：日期格子停用、保留 border 標示、點擊不觸發選取', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 5, 12, 17, 30)) // 17:30，晚於窗口結束 16:00
    ...
    const wrapper = mount(AvailabilityPickerModal, {
      props: {
        ...
        rangeStart: '2026-06-11',
        rangeEnd: '2026-06-17',
        allowedDates: ['2026-06-12', '2026-06-13'],
        dateWindows: {
          '2026-06-12': { start: '14:00', end: '16:00', slotId: 'slot-1' },
          '2026-06-13': { start: '14:00', end: '16:00', slotId: 'slot-2' },
        },
      },
      global: { plugins: [createTestI18n()], stubs: { Teleport: true } },
    })

    const day12 = findDayCell(wrapper, 12)
    expect(day12.classes()).toContain('opacity-50')
    expect(day12.classes()).toContain('cursor-not-allowed')
    // border 保留（非 border-transparent），用來跟「從來不是候選日」的格子區分開
    expect(day12.classes()).toContain('border-[var(--bujo-line-soft)]')
    expect(day12.classes()).not.toContain('border-transparent')

    await day12.trigger('mousedown')
    window.dispatchEvent(new Event('mouseup'))
    await wrapper.vm.$nextTick()

    expect(wrapper.vm.selectedDates['2026-06-12']).toBeUndefined()
  })

  test('情境二/三全域時間窗（timeWindowStart/timeWindowEnd 中較晚者）已過去：日期格子停用', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 5, 12, 19, 0)) // 19:00，晚於全域時間窗結束 18:00
  ...
  test('沒有任何窗口限制的日期，用當天 23:59 當過期邊界：已經是隔天則停用', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 5, 13, 9, 0)) // 現在是 6/13，6/12 早就過了 23:59
  ...
  test('未過期的日期維持可選取，不受影響（沒有停用樣式，點擊正常選取）', async () => {
    vi.useFakeTimers()
    ...
    const day12 = findDayCell(wrapper, 12)
    expect(day12.classes()).not.toContain('opacity-50')
    expect(day12.classes()).not.toContain('cursor-not-allowed')
