...
describe('EventPage - 活動名稱長度限制', () => {
  ...
  test('超過 15 字時顯示驗證錯誤且不送出', async () => {
    const calls = stubActivitiesFetch()
    const wrapper = await mountEventPage()

    wrapper.vm.form.name = '1234567890123456'
    await flushPromises()
    queryBody('button[form="event-form"][type="submit"]').click()
    await flushPromises()

    expect(wrapper.vm.submitError).toBe('活動名稱最多 15 字')
    expect(calls.find((c) => c.url.includes('/api/activities'))).toBeUndefined()

    wrapper.unmount()
  })

  test('剛好 15 字時可送出，payload 使用 trim 後標題', async () => {
    const calls = stubActivitiesFetch()
    const wrapper = await mountEventPage()
    ...
    wrapper.vm.form.name = '  123456789012345  '
    await flushPromises()
    queryBody('button[form="event-form"][type="submit"]').click()
    await flushPromises()
    ...
    expect(JSON.parse(call.options.body).title).toBe('123456789012345')
...
describe('EventPage - 情境二（日期確定、時間未確定）表單簡化', () => {
  beforeEach(() => {
    // 固定在早上，跟這些測試裡挑選的時段（09:00／14:00 等）都離「現在」夠遠，
  ...
  test('只填時段範圍開始時間、結束時間留空時顯示行內驗證錯誤、不送出', async () => {
    ...
    wrapper.vm.timeWindow.startTime = '09:00'
  ...
  test('完整設定合法時間窗時正常送出，payload 含 timeWindowStart/timeWindowEnd', async () => {
    ...
    await flushPromises()
    clickTimeOption('時段範圍開始時間選單', '09:00')
    await flushPromises()
...
describe('EventPage - 情境二/三/四開始時間選單排除今天已過去時段', () => {
  ...
  test('情境三：今天的日期存在於 candidateDates 時，統一開始時間選單排除已過去的小時', async () => {
    ...
    expect(wrapper.vm.uniformStartTimeOptions.map((t) => t)).not.toContain('09:00')
    expect(wrapper.vm.uniformStartTimeOptions[0]).toBe('10:00')
  ...
  test('情境三：candidateDates 都不是今天時，統一開始時間選單不受目前時間影響', async () => {
    ...
    expect(wrapper.vm.uniformStartTimeOptions[0]).toBe('00:00')
  ...
  test('情境四：候選時段自己的日期是今天時，該時段的開始時間選單排除已過去的小時，不受其他候選時段日期影響', async () => {
    ...
    expect(wrapper.vm.slotStartTimeOptions('2026/07/15')).not.toContain('09:00')
    expect(wrapper.vm.slotStartTimeOptions('2026/07/15')[0]).toBe('10:00')
    expect(wrapper.vm.slotStartTimeOptions('2026/07/20')[0]).toBe('00:00')
...
describe('EventPage - 情境三：候選日期包含今天時，今天這一格的鎖定規則', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    // 14:00：早於 14 點的小時（例如09:00）對今天而言已經過去，晚於的（例如15:00）還有效
  ...
  test('先選未來日期並設定一個對今天已過期的統一時間，再點今天：今天格子被鎖住，且已選的時間不受影響', async () => {
    ...
    wrapper.vm.uniformTime.startTime = '09:00'
    wrapper.vm.uniformTime.endTime = '10:00'
    await flushPromises()
    ...
    expect(wrapper.vm.uniformTime.startTime).toBe('09:00')
    expect(wrapper.vm.uniformTime.endTime).toBe('10:00')
  ...
  test('已選的統一時間對今天而言仍有效時，今天格子可以正常點選', async () => {
    ...
    wrapper.vm.timeMode = 'fixed'
    wrapper.vm.candidateDates = ['2026/07/20']
    wrapper.vm.uniformTime.startTime = '15:00'
    wrapper.vm.uniformTime.endTime = '16:00'
    await flushPromises()
...
describe('EventPage - 月曆格子與結束時間過濾（重構前安全網）', () => {
  ...
  test('情境二 timeWindowEndTimeOptions：排除不晚於已選開始時間的選項', async () => {
    ...
    expect(wrapper.vm.timeWindowEndTimeOptions).not.toContain('09:00')
    expect(wrapper.vm.timeWindowEndTimeOptions[0]).toBe('10:00')
  ...
  test('情境三 uniformEndTimeOptions：排除不晚於已選開始時間的選項', async () => {
    ...
    expect(wrapper.vm.uniformEndTimeOptions).not.toContain('09:00')
    expect(wrapper.vm.uniformEndTimeOptions[0]).toBe('10:00')
  ...
  test('情境四 slotEndTimeOptions：排除不晚於該時段自己已選開始時間的選項', async () => {
    ...
    const options = wrapper.vm.slotEndTimeOptions({ startTime: '09:00', endTime: null })
...
describe('EventPage - 底部常駐顯示：報名截止時間套用警示樣式（決策硬截止時間不再有對應文字，只驅動二次確認彈窗）', () => {
  afterEach(() => {
    vi.useRealTimers()
  })

  test('活動時間夠遠：顯示一般文案，不套用警示樣式', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    queryBody('#event-start-date').click()
  ...
  test('報名截止時間警示：情境一活動距今 30 分鐘，智慧演算法降級到無報名緩衝，套用警示樣式', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    queryBody('#event-start-time').click()
  ...
  test('決策硬截止時間本身貼近現在，但報名截止時間刻意拉遠：兩個警示旗標各自獨立，不是同一個判斷共用出來的結果', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    queryBody('#event-start-date').click()
    ...
    // 遠期活動：isReportCutoffWarning／isScheduleCeilingWarning 目前都是 false，
    // 證明兩者是各自獨立的 computed，不是同一個判斷共用出來的結果
  ...
  test('報名截止時間警示：情境二時段範圍開始時間設在 30 分鐘後', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    await enterScenario2()
...
describe('EventPage - 尚未選完日期/時間時，報名截止提醒完全不顯示（回歸測試：修正誤把「還沒選」當成「已經過期」的 bug）', () => {
  afterEach(() => {
    vi.useRealTimers()
  })

  test('情境一：剛開啟表單、還沒選開始時間，不顯示提醒也不套用警示樣式', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    await flushPromises()
    ...
    expect(document.body.textContent).not.toContain('距離報名截止僅剩')
    expect(wrapper.vm.isReportCutoffWarning).toBe(false)
    expect(wrapper.vm.scheduleCeilingDate).toBeNull()

    wrapper.unmount()
  })

  test('情境二：切到情境二、還沒設定時段範圍，不顯示提醒', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    await enterScenario2()
  ...
  test('情境三：切到情境三、還沒選任何候選日期，不顯示提醒', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    wrapper.vm.dateMode = 'range'
  ...
  test('情境四：切到情境四、還沒設定任何候選時段，不顯示提醒', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const wrapper = await mountEventPage()
    wrapper.vm.dateMode = 'range'
...
describe('EventPage - 選好開始時間後結束時間自動帶入 +1 小時', () => {
  ...
  test('情境二時段範圍：選開始時間自動帶入結束時間；手動選過結束時間後，改開始時間不會覆蓋', async () => {
    ...
    wrapper.vm.selectSlotTime(wrapper.vm.timeWindow, 'startTime', '09:00')
    expect(wrapper.vm.timeWindow.endTime).toBe('10:00')

    wrapper.vm.selectSlotTime(wrapper.vm.timeWindow, 'endTime', '15:00')
    wrapper.vm.selectSlotTime(wrapper.vm.timeWindow, 'startTime', '10:00')
    expect(wrapper.vm.timeWindow.endTime).toBe('15:00')
  ...
  test('情境三統一時間：選開始時間自動帶入結束時間；手動選過結束時間後，改開始時間不會覆蓋', async () => {
    ...
    wrapper.vm.selectSlotTime(wrapper.vm.uniformTime, 'startTime', '09:00')
    expect(wrapper.vm.uniformTime.endTime).toBe('10:00')

    wrapper.vm.selectSlotTime(wrapper.vm.uniformTime, 'endTime', '15:00')
    wrapper.vm.selectSlotTime(wrapper.vm.uniformTime, 'startTime', '10:00')
    expect(wrapper.vm.uniformTime.endTime).toBe('15:00')
  ...
  test('情境四候選時段：選開始時間自動帶入結束時間；手動選過結束時間後，改開始時間不會覆蓋', async () => {
    ...
    wrapper.vm.selectSlotTime(slot, 'startTime', '09:00')
    expect(slot.endTime).toBe('10:00')

    wrapper.vm.selectSlotTime(slot, 'endTime', '15:00')
    wrapper.vm.selectSlotTime(slot, 'startTime', '10:00')
  ...
  test('開始時間選在 23:00 時，自動帶入的結束時間是隔天 00:00', async () => {
    ...
    const slot = { startTime: null, endTime: null }
    wrapper.vm.selectSlotTime(slot, 'startTime', '23:00')
...
describe('EventPage - 送出前驗證流團時間仍在未來', () => {
  ...
  test('選好預設後、送出前時間已經過去，導致計算出的 deadline 不再晚於現在時，中止送出並顯示錯誤', async () => {
    ...
    expect(document.body.textContent).toContain('報名截止時間已經過去，請重新調整活動日期或時間')
    expect(calls.find((c) => c.url.includes('/api/activities'))).toBeUndefined()
...
describe('EventPage - 二次確認 modal：只有決策硬截止時間本身貼近現在才攔截送出', () => {
  ...
  test('決策硬截止時間距今 ≤30 分鐘時，送出會先跳確認 modal，不直接送出', async () => {
    ...
    expect(document.body.textContent).toContain('距離活動開始只剩')
    expect(document.body.textContent).toContain('分鐘')
    expect(document.body.textContent).toContain('確定要建立活動嗎？')
    expect(calls.find((c) => c.url.includes('/api/activities'))).toBeUndefined()
...
describe('EventPage - initialDate prop 對過去日期不套用（防禦性第二層，主要防護在 DateEventsModal.vue 的「+」按鈕）', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15))
  })
  ...
  async function mountWithInitialDate(initialDate) {
    const router = createRouter({
      history: createMemoryHistory(),
      routes: [
        { path: '/', component: { template: '<div />' } },
    ...
    // 比照 CalendarMain.vue 的實際用法：EventPage 常駐掛載、isOpen 從 false 切成 true 才會觸發
  ...
  test('過去日期不套用，表單維持預設的今天日期，情境模式不被改變', async () => {
    const wrapper = await mountWithInitialDate('2026-07-10')
    await flushPromises()

    expect(wrapper.vm.form.startDate).toBe('2026/07/15')
    expect(wrapper.vm.form.endDate).toBe('2026/07/15')
    expect(wrapper.vm.form.singleDate).toBe('2026/07/15')
    expect(wrapper.vm.dateMode).toBe('fixed')
    expect(wrapper.vm.timeMode).toBe('fixed')
  ...
  test('今天或未來日期正常套用（行為與現有一致）', async () => {
    const wrapper = await mountWithInitialDate('2026-07-20')
    await flushPromises()

    expect(wrapper.vm.form.startDate).toBe('2026/07/20')
    expect(wrapper.vm.form.endDate).toBe('2026/07/20')
    expect(wrapper.vm.form.singleDate).toBe('2026/07/20')
    expect(wrapper.vm.dateMode).toBe('fixed')
    expect(wrapper.vm.timeMode).toBe('fixed')
...
describe('EventPage - 送出前擋下「報名截止時間已經過去」時的錯誤訊息文案', () => {
  afterEach(() => {
    vi.useRealTimers()
  })

  test('選定時是安全的未來時間，但填表單拖到報名截止時間已過才送出，顯示新版文案', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 6, 15, 9, 30))
    const calls = stubActivitiesFetch()
    const wrapper = await mountEventPage()
    ...
    // 選定當下（7/15 09:30），預設演算法會選到安全的「3 小時前」，
    // 報名截止時間落在 7/16 07:00，離選定當下還有將近 22 小時，不是警示狀態
    expect(wrapper.vm.isReportCutoffWarning).toBe(false)

    // 模擬使用者填表單拖了一段時間——時間推進到 7/16 08:00，
    // 已經晚於前面選定當下算出的報名截止時間（7/16 07:00）
    vi.setSystemTime(new Date(2026, 6, 16, 8, 0))

    setInputValue(queryBody('#event-name'), '截止過期測試')
    await flushPromises()
    queryBody('button[form="event-form"][type="submit"]').click()
    await flushPromises()

    expect(wrapper.vm.submitError).toBe('報名截止時間已經過去，請重新調整活動日期或時間')
    expect(calls.find((c) => c.url.includes('/api/activities'))).toBeUndefined()
