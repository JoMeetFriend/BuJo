...
describe('ActivityDetailModal - 情境二三四(requires_voting) 揪團中提前手動成團', () => {
  test('建立者在 recruiting 狀態下可看到候選時段票數並選取後啟用「立即成團」按鈕', async () => {
  ...
  test('點擊「立即成團」會呼叫 confirm-formation 並帶上選取的候選時段 id', async () => {
    const activity = makeActivity()
...
describe('ActivityDetailModal - 頭像相對路徑要補上 API base URL，避免破圖', () => {
  ...
  test('建立者頭像圖片載入失敗時改顯示預設圖示', async () => {
    const activity = makeActivity({
      creator: {
        id: 'creator-1',
        display_name: '小明',
    ...
    const creator = wrapper.find('.activity-detail-creator')
    expect(creator.find('img').exists()).toBe(true)

    await creator.get('img').trigger('error')

    expect(creator.find('img').exists()).toBe(false)
    expect(creator.find('svg').exists()).toBe(true)
  })

  test('參與者頭像圖片載入失敗時改顯示姓名縮寫', async () => {
    const activity = makeActivity({
      participants: [
        {
          id: 'p-1',
    ...
    const avatars = wrapper.find('.activity-detail-avatars')
    expect(avatars.find('img').exists()).toBe(true)

    await avatars.get('img').trigger('error')

    expect(avatars.find('img').exists()).toBe(false)
    expect(avatars.text()).toContain('小華'.slice(0, 2))
  })
})
...
describe('ActivityDetailModal - Scenario C 日期-only 報名流程', () => {
  ...
  test('已成團（confirmed_slot 存在）時不顯示「日期投票中」', async () => {
    ...
    expect(wrapper.text()).toContain('8/1   19:00 – 21:00')
    expect(wrapper.text()).not.toContain('日期＆時間')
    const labels = wrapper.findAll('.activity-detail-label').map((label) => label.text())
    expect(labels).not.toContain('日期')
    expect(labels).not.toContain('時間')
  })
...
describe('ActivityDetailModal - 情境二只有唯一最高票時自動預選', () => {
  test('情境二只有一筆最高票時，自動預選該筆，「立即成團」按鈕直接可用', async () => {
    const activity = makeRangeActivity({
      requires_voting: true,
      is_creator: true,
      status: 'recruiting',
