...
describe('ActivityDetailModal - voting 狀態下，決選候選清單只有建立者看得到', () => {
  test('非建立者在 voting 狀態看不到候選時段清單，footer 顯示「已報名」', async () => {
    ...
    expect(wrapper.text()).not.toContain('候選時段（依票數排序')
    expect(wrapper.find('input[type="radio"]').exists()).toBe(false)
...
describe('ActivityDetailModal - 決策項目比例文字（X/Y人）', () => {
  ...
  test('沒有其他真人參與者（分母為 0）時只顯示人數，不顯示 X/0人', async () => {
    const activity = makeActivity({
    ...
    expect(wrapper.text()).toContain('0人')
  })
})
...
describe('ActivityDetailModal - Scenario D 各候選時段的展開狀態互不影響', () => {
  test('展開其中一個候選時段的清單，不影響另一個候選時段目前顯示的筆數', async () => {
    const activity = makeScenarioDActivity({
      is_creator: true,
      status: 'voting',
      decision_candidates: [
    ...
    expect(wrapper.findAll('.activity-detail-option')).toHaveLength(4) // slot-a 前 3 筆 + slot-b 1 筆

    const moreButton = wrapper.findAll('button').find((b) => b.text().includes('顯示更多'))
    await moreButton.trigger('click')

    // slot-a 展開到 6 筆，slot-b 維持 1 筆（沒有自己的顯示更多按鈕），不受影響
    expect(wrapper.findAll('.activity-detail-option')).toHaveLength(7)
  })
})
...
describe('ActivityDetailModal - 情境四候選時段外層群組標題只顯示日期，hover/長按顯示完整時間；個別投票列維持直接顯示', () => {
  function makeGroupHeaderActivity() {
    return makeScenarioDActivity({
      is_creator: true,
      status: 'voting',
      decision_candidates: [
        {
          id: 'slot-a',
          slot_start: '2026-07-31T05:00:00',
          slot_end: '2026-07-31T21:00:00',
          count: 1,
          segments: [
            {
              id: 'seg-1',
              slot_start: '2026-07-31T08:00:00',
              slot_end: '2026-07-31T09:00:00',
              count: 1,
              is_unanimous: true,
              supporters: [],
            },
  ...
  test('群組標題預設只顯示日期，個別投票列仍直接顯示完整時間', async () => {
    const activity = makeGroupHeaderActivity()
    stubFetch(activity)

    const wrapper = mount(ActivityDetailModal, { props: { isOpen: true, activityId: 'act-1' } })
    await flushPromises()

    expect(wrapper.find('.activity-detail-group-label').text()).toBe('7/31')
    // 底下實際可投票的那一列時間是重點資訊，一律直接顯示，不受群組標題收合影響
    expect(wrapper.text()).toContain('上午 8:00')
  })

  test('桌面 hover 群組標題顯示完整時間，mouseleave 後隱藏', async () => {
    const activity = makeGroupHeaderActivity()
    stubFetch(activity)
    ...
    const groupLabel = wrapper.find('.activity-detail-group-label')
    await groupLabel.trigger('mouseenter')
    expect(wrapper.find('.activity-detail-chip-popover').text()).toContain('下午 9:00')

    await groupLabel.trigger('mouseleave')
    expect(wrapper.find('.activity-detail-chip-popover').exists()).toBe(false)
  })

  test('行動裝置長按群組標題滿 500ms 顯示完整時間', async () => {
    vi.useFakeTimers()
    const activity = makeGroupHeaderActivity()
    stubFetch(activity)
    ...
    const groupLabel = wrapper.find('.activity-detail-group-label')
    await groupLabel.trigger('touchstart')
    vi.advanceTimersByTime(500)
    await wrapper.vm.$nextTick()
...
describe('ActivityDetailModal - 決選候選清單只渲染給建立者', () => {
  it('非建立者在 voting 狀態下看不到決選候選清單；建立者不受影響', async () => {
    ...
    expect(participantWrapper.text()).not.toContain('候選時段（依票數排序')
    ...
    expect(creatorWrapper.text()).toContain('候選時段（依票數排序')
  })
})

describe('ActivityDetailModal - 三個唯讀區塊新增 co_participants 頭像列', () => {
  it('情境二「你已回報的時間」：co_participants 頭像預設顯示，hover 顯示姓名，空陣列不渲染頭像', async () => {
    const activity = makeRangeActivity({
      is_creator: false,
      has_joined: true,
      status: 'recruiting',
    ...
    expect(entries[1].find('.activity-detail-avatar').exists()).toBe(false)

    await entries[0].find('.activity-detail-supporters').trigger('mouseenter')
    expect(wrapper.find('.activity-detail-chip-popover').text()).toContain('Bob')
  })

  it('情境三「你已選擇的日期」：co_participants 頭像預設顯示，hover 顯示姓名', async () => {
    const activity = makeScenarioCActivity({
      is_creator: false,
      has_joined: true,
      status: 'recruiting',
  ...
  it('情境四「你已選擇的候選時段」：co_participants 頭像預設顯示，長按顯示姓名', async () => {
    vi.useFakeTimers()
    const activity = makeScenarioDActivity({
      is_creator: false,
      has_joined: true,
