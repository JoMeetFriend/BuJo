...
describe('ActivityDetailModal - 情境二三四(requires_voting) 揪團中提前手動成團', () => {
  test('建立者在 recruiting 狀態下可看到候選時段票數並選取後啟用「提前成團」按鈕', async () => {
    ...
    const activity = makeActivity({
      decision_candidates: [
        { id: 'slot-a', slot_start: '2026-07-10T10:00:00Z', slot_end: '2026-07-10T12:00:00Z', count: 1 },
...
describe('ActivityDetailModal - Scenario D 候選時段窗口報名流程', () => {
  ...
  test('handleScenarioDConfirm 正確把 range 對應回 slotId，用正確欄位名稱送出 candidateSlotRanges', async () => {
    ...
    expect(JSON.parse(joinCall.options.body)).toEqual({
      ...
      candidateSlotRanges: [
        { candidateSlotId: 'slot-a', rangeStart: '2026-08-01T14:00:00', rangeEnd: '2026-08-01T16:00:00' },
        { candidateSlotId: 'slot-b', rangeStart: '2026-08-03T09:00:00', rangeEnd: '2026-08-03T12:00:00' },
      ],
    })
  })
...
describe('ActivityDetailModal - Scenario D 建立者決策畫面巢狀交集顯示', () => {
      ...
      decision_candidates: [
        {
          ...
          count: 2,
          perfect_overlap: [
            { id: 'temp-2026-08-01T14:00:00.000Z', slot_start: '2026-08-01T14:00:00', slot_end: '2026-08-01T15:00:00', count: 2 },
          ],
          partial_overlap: [
            { id: 'temp-2026-08-01T15:00:00.000Z', slot_start: '2026-08-01T15:00:00', slot_end: '2026-08-01T16:00:00', count: 1 },
          ],
        },
        {
          id: 'slot-b',
          ...
          perfect_overlap: [],
          partial_overlap: [
            { id: 'temp-2026-08-03T09:00:00.000Z', slot_start: '2026-08-03T09:00:00', slot_end: '2026-08-03T10:00:00', count: 1 },
          ],
        },
      ],
      ...overrides,
  ...
  test('選定某個候選時段底下的窄窗口並確認時，正確送出 candidateSlotId+slotStart+slotEnd', async () => {
    ...
    const target = radios.find((r) => r.attributes('value') === 'slot-b::temp-2026-08-03T09:00:00.000Z')
